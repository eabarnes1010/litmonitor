...
**Sidebar**: everything that changes the next run — fetch settings, journals, custom sources, API key — with **Cost estimate** and **Clear cache** at the bottom. Its top section does not scroll, so **Fetch & Score** and the status line stay in reach. The **status line** reports the last run in one clamped line; **click it** for the full per-source digest of that run (see *Odds and ends*).
...
| --- | --- | --- |
| **Fetch days back** | 2 | A slider over 1–30 days with **Auto** at its left stop, or type any window up to 99 into the box. **Auto** asks each source for everything since *that source* was last fetched in full, so a feed that has been failing for a fortnight is caught up on the next run that works while the sources that were fine keep the recent window. Coverage is recorded per source, up to the moment a source answered in full — no error, no partial, nothing trimmed by the 1,000-paper cap. An arXiv source is the one exception: it is fetched as an announcement feed plus a date window, and if only its feed drops (a large category's announcement can be too big for the free relays) it is still recorded as covered up to the newest submission date its window returned, so one chronically flaky feed cannot keep widening the window for every source. Journals are asked what Crossref has newly *indexed* since that mark rather than what was published, because a paper is indexed days to weeks after its publication date; arXiv is asked by submission date with a four-day overlap. Catching up costs no extra tokens — papers already scored under the current profile are cache hits, and Auto follows when a paper newly *appeared* to you (when Crossref indexed it, or when it was posted to arXiv) rather than its publication date. A typed number stays a literal window with no overlap added, and it counts by **publication date** for a journal — handy for pulling older papers from a source you just added — and by **posting date** for arXiv. |
...
Sources are tagged **api** (journals, read directly from Crossref, with OpenAlex as a per-source fallback) or **relay** (arXiv, which a browser cannot read directly).
...
- **Messages** come in two colours. Red is a failure — storage full, a run that died, a backup file that could not be read — and always needs you. Amber is a notice: nothing is broken, and it is either an empty state or a report on the run. The three notices a run writes about itself (a source that did not answer, a source that answered for part of its window, the paper cap) carry a **Don't show again** button that silences that kind for good, whichever source it is about next time. That is safe because a source that failed or came back partial does not lose the days it never covered — a failed source keeps its coverage mark, and a partial arXiv source advances only as far as its date window actually reached — so the uncovered window is asked for again on the next run whether or not the run says so.
- **Fetch digest.** Clicking the status line opens a plain-text digest of the most recent run: how many papers each selected source returned, with any source that **failed** or came back **partial** flagged, how many of its papers were already in the score **cache**, and — when the Haiku prefilter ran — how many it **cut**. Cache hits skip the prefilter, so a source's "cut by Haiku" is over its uncached papers only, and its "cached" figure accounts for the rest. A footer gives the run totals (fetched, after dedup and filtering, from cache, dropped by Haiku, scored). It is counts only, kept so you can see what happened behind the scenes and spot a source quietly returning nothing. It is written on every finished run, including one that found nothing, and survives a reload.
